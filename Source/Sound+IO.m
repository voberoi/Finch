...
- (ReadInfo) readPCMDataFrom: (NSURL*) path
{
    ...
    errcode = AudioFileReadBytes(fileId, false, 0, &dataSize, result.data);
		if (errcode) {
			NSLog(@"Failed to read sound data.", errcode);
			goto exit;
		}
	} else {
		// Future work uncompress and reinterpret the sound data as PCM
		NSLog(@"Only files PCM data supported.");
		goto exit;
	}
	...
	if (!TestAudioFormatNativeEndian(fileFormat))
	{
		if (fileFormat.mBitsPerChannel == 16) {
			for (int i =0; i< dataSize; i+=2) {
				UInt8 b = *(UInt8 *)(result.data+i);
				*(UInt8 *)(result.data+i) = *(UInt8 *)(result.data+i+1);
				*(UInt8 *)(result.data+i+1) = b;
			}
		}
		fileFormat.mFormatFlags = fileFormat.mFormatFlags & ~kAudioFormatFlagIsBigEndian;
	}
	
	if (errcode == noErr) {
		result.success = YES;
		result.bytesRead = dataSize;
		result.dataFormat = (fileFormat.mChannelsPerFrame > 1) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
...
exit:
	if (errcode != noErr)	NSLog(@"Failed to read sound data (%x).", errcode);
	if (fileId)
